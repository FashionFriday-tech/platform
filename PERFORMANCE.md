...
const GiftCardModal = dynamic(() => import('@/features/gift-cards/gift-cards-page'), {
  loading: () => <Skeleton />,
});
...
```tsx
const WalletPage = dynamic(() => import('@/features/wallet').then((mod) => mod.WalletPage));
```
...
```tsx
dynamic(() => import(/* webpackChunkName: "referrals" */ '@/features/referrals'));
```
