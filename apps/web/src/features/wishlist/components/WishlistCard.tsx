...
export function WishlistCard({ product }: WishlistCardProps) {
  ...
  return (
    <article className="border-border rounded-4xl group relative flex w-full items-center justify-center gap-4 border-y py-6">
      ...
      <div className="bg-background-muted relative aspect-square w-40 shrink-0 overflow-hidden rounded-2xl sm:w-32 md:w-40">
        ...
        {!product.inStock && (
          <div className="bg-background/40 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-[1px]">
            <div className="bg-background/80 text-foreground rounded-full px-2 py-1 text-[8px] font-bold uppercase tracking-tighter sm:text-[10px]">
              Sold Out
            </div>
          </div>
        )}
        ...
        {discount > 0 && (
          <div className="absolute left-2 top-2">
            <span className="bg-background text-foreground rounded-full px-2 py-0.5 text-[8px] font-bold uppercase tracking-tighter sm:text-[10px]">
              -{discount}%
            </span>
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col justify-between self-stretch py-1">
        <div className="flex justify-between gap-2">
          <div className="min-w-0 flex-1">
            <p className="text-foreground-subtle text-[10px] font-medium uppercase tracking-widest">
              {product.category}
            </p>
            <h3 className="text-foreground group-hover:text-foreground-muted mt-1 text-sm font-semibold leading-tight transition-colors sm:text-base">
        ...
        <div className="mt-auto flex w-full items-center justify-between gap-2 pr-2">
          ...
          <button
            ...
            className="bg-foreground text-background flex w-full items-center justify-center gap-2 rounded-full py-2.5 text-[10px] font-bold uppercase tracking-widest shadow-sm transition-transform active:scale-95 sm:px-6 sm:py-2.5 sm:text-xs"
          >
            {product.inStock ? (
              <>
