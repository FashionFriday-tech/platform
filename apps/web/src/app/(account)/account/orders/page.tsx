...
import { type Order, type OrderItem, orders, type OrderStatus } from '@/data/order'; // Assuming OrderItem is exported there
...
const StatusBadge = ({ status, label }: { status: OrderStatus; label: string }) => {
  ...
  return (
    <span
      className={`rounded-full border px-3 py-1 text-[10px] font-bold tracking-widest uppercase ${getStatusColor(status)}`}
    >
      {label}
    </span>
  );
