      ...
      <div className="flex w-full max-w-lg flex-col overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-[#111111]">
        ...
        <div className="flex items-center justify-between border-b border-black/5 p-6 dark:border-white/5">
          <div>
            ...
            <p className="mt-1 text-sm text-black/60 dark:text-white/60">Send an invitation to join the admin workspace.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 p-6">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-black dark:text-white">Email Address</label>
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full rounded-xl border border-black/10 bg-transparent py-3 pl-12 pr-4 text-sm text-black outline-none focus:border-black/30 dark:border-white/10 dark:text-white dark:focus:border-white/30"
              />
            </div>
          </div>
              ...
              {roles.map((role) => (
                ...
                >
                  <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${
                    selectedRole === role ? 'border-black dark:border-white' : 'border-black/20 dark:border-white/20'
                  }`}>
                    {selectedRole === role && <div className="h-2.5 w-2.5 rounded-full bg-black dark:bg-white" />}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-black dark:text-white">{ROLE_LABELS[role]}</span>
                      {role === 'SUPER_ADMIN' && <ShieldCheckIcon className="h-4 w-4 text-orange-500" />}
                    </div>
                    <p className="mt-1 text-xs text-black/60 dark:text-white/60">{ROLE_DESCRIPTIONS[role]}</p>
                  </div>
                </button>
              ))}
            </div>
