...
import { PackageIcon, StarIcon, EditIcon, CheckIcon, TrashIcon } from '@ff/ui';
...
export function CategoryProductTable({ products, onRemoveProduct }: CategoryProductTableProps) {
  ...
  if (products.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-black/10 py-12 dark:border-white/10">
        ...
        <p className="text-sm font-medium text-black/60 dark:text-white/60">No products in this category</p>
      </div>
    );
  }
          ...
          {products.map((product) => (
            ...
            >
              ...
              <td className="px-6 py-4 text-right">
                <div className={`flex items-center justify-end gap-2 transition-opacity duration-200 ${hoveredRow === product.id ? 'opacity-100' : 'opacity-0'}`}>
                  <button
