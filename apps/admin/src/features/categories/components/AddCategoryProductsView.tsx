...
export function AddCategoryProductsView({ category }: AddCategoryProductsViewProps) {
  ...
  const handleSelectAllVisible = () => {
    const assignable = filteredProducts.filter((p) => p.categoryId !== category.id);
    const newSelected = new Set(selectedProductIds);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Error saving products to category:', err);
      ...
      <div className="flex flex-col gap-4 border-b border-black/5 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/5">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="rounded-full bg-black/5 px-2.5 py-0.5 text-xs font-semibold text-black/60 dark:bg-white/5 dark:text-white/60">
              {category.gender}
            </span>
          </div>
          <p className="mt-1 text-xs text-black/50 dark:text-white/50">
            Click on products to select them, then click Add to assign them to {category.name}.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {filteredProducts.map((product) => {
              const isAlreadyInThisCategory = product.categoryId === category.id;
