...
export function AddCollectionProductsView({ collection }: AddCollectionProductsViewProps) {
  ...
  // Simulate existing assigned products based on collection id
  const existingAssignedProductIds = useMemo(() => {
    return new Set(mockProducts.slice(0, 10).map((p) => p.id));
  }, [collection.id]);
    ...
    <div className="scrollbar-hide flex h-full flex-col gap-6 overflow-hidden">
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          ...
          className="w-full rounded-xl border border-black/5 bg-[#f8f9fa] py-3 pr-4 pl-10 text-sm text-black placeholder-black/40 outline-none focus:border-black/20 focus:bg-white dark:border-white/5 dark:bg-[#1a1a1a] dark:text-white dark:placeholder-white/40 dark:focus:border-white/20 dark:focus:bg-[#222222]"
        />
      </div>
