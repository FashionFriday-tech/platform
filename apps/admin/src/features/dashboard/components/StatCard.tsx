...
export function StatCard({ title, value, subtitle, icon: Icon, iconColorClass, iconBgClass, href, trend }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute -right-10 -top-10 h-32 w-32 rounded-full blur-3xl transition-opacity group-hover:opacity-100 opacity-30 ${iconBgClass.replace('bg-', 'bg-')}`} />
      ...
      <div className="relative z-10 flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm font-bold tracking-wider text-black/70 uppercase dark:text-white/70">{title}</p>
        </div>
        ...
        <div>
          ...
          <p className={`mt-2 text-sm font-medium ${
            ...
            {subtitle} {href && <span className="ml-1 opacity-0 transition-opacity group-hover:opacity-100">→</span>}
          </p>
        </div>
      </div>
    </CardWrapper>
