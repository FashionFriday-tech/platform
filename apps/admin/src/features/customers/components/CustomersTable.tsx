...
export function CustomersTable({
  ...
  const getStatusBadge = (status: 'active' | 'blocked') => {
    if (status === 'active') {
      return (
        <span className="inline-flex min-w-[80px] items-center justify-center rounded-md bg-green-50 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-green-600/20 ring-inset dark:bg-green-500/10 dark:text-green-400 dark:ring-green-500/20">
          Active
        </span>
      );
    }
    return (
      <span className="inline-flex min-w-[80px] items-center justify-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-red-600/20 ring-inset dark:bg-red-500/10 dark:text-red-400 dark:ring-red-500/20">
        Blocked
      </span>
    );
  };
      ...
      <div className="scrollbar-hide flex-1 overflow-auto">
        <table className="relative w-full text-left text-sm text-black dark:text-white">
          <thead className="sticky top-0 z-30 border-b border-black/5 bg-[#f8f9fa] text-xs font-medium text-black/60 uppercase dark:border-white/5 dark:bg-[#1a1a1a] dark:text-white/60">
            <tr>
              <th
                className="sticky left-0 z-20 min-w-[250px] cursor-pointer bg-[#f8f9fa] px-6 py-4 whitespace-nowrap transition-colors hover:bg-[#e9ecef] dark:bg-[#1a1a1a] dark:hover:bg-[#2a2a2a]"
                onClick={() => {
                  onSort('name');
                }}
              >
            ...
            {customers.map((customer, index) => (
              <tr
                ...
                className={twMerge(
                  ...
                  openDropdown === customer.id ? 'z-20 bg-gray-50 dark:bg-white/5' : '',
                )}
              >
                <td className="sticky left-0 z-10 min-w-[250px] bg-white px-6 py-4 whitespace-nowrap group-hover:bg-gray-50 dark:bg-[#111111] dark:group-hover:bg-[#1a1a1a]">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-black/60 group-hover:bg-black/[0.02] dark:text-white/60 dark:group-hover:bg-white/[0.02]">
                  {customer.email}
                </td>
                ...
                <td
                  className={twMerge(
                    ...
                    openDropdown === customer.id ? 'z-30' : 'z-10',
                  )}
                >
                  <button
                    onClick={(e) => {
