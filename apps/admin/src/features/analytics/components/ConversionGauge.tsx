    ...
    <div className="flex h-[460px] w-full flex-col rounded-2xl border border-black/[0.06] bg-white shadow-xl shadow-black/[0.04] dark:border-white/[0.08] dark:bg-[#1a1a1a]">
      <div className="flex items-start justify-between border-b border-black/[0.04] p-6 pb-4 dark:border-white/[0.04]">
        <div>
          ...
          <p className="text-sm font-medium text-black/40 dark:text-white/40">Visit → Purchase rate</p>
        </div>
      ...
      <div className="relative flex flex-1 items-center justify-center px-6 pt-2 pb-0">
        ...
        <div className="pointer-events-none absolute bottom-1 flex flex-col items-center">
          <span className="text-2xl font-black tracking-tight text-black dark:text-white">{conversionRate}%</span>
          <span className="mt-0.5 text-[10px] font-bold uppercase tracking-widest text-black/30 dark:text-white/30">Rate</span>
        </div>
      </div>
      ...
      <div className="flex flex-col justify-center gap-1.5 border-t border-black/[0.04] px-5 py-3 pb-4 dark:border-white/[0.04]">
        {funnelSteps.map((step, i) => {
          ...
          return (
            <div key={step.label} className="group flex items-center gap-3 rounded-lg px-2 py-1.5 transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">
              <div
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-black/70 dark:text-white/70">{step.label}</span>
