...
export function Header() {
  ...
  const getPageInfo = () => {
    if (pathname === '/') {
      return { title: 'Dashboard', showBack: false, icon: ActivityIcon };
    }
    if (pathname.startsWith('/orders/')) {
      return { title: 'Order Details', showBack: true, icon: ShoppingBagIcon };
    }
    if (pathname === '/orders') {
      return { title: 'Orders Management', showBack: false, icon: ShoppingBagIcon };
    }
    if (pathname.startsWith('/products/')) {
      return { title: 'Product Details', showBack: true, icon: PackageIcon };
    }
    if (pathname === '/products') {
      return { title: 'Products', showBack: false, icon: PackageIcon };
    }
    if (pathname === '/customers') {
      return { title: 'Customers', showBack: false, icon: UsersIcon };
    }
    if (pathname === '/reviews') {
      return { title: 'Reviews', showBack: false, icon: StarIcon };
    }
    if (pathname === '/brands') {
      return { title: 'Brands', showBack: false, icon: StarBadgeIcon };
    }
    if (pathname.startsWith('/categories/')) {
      return { title: 'Category Details', showBack: true, icon: CategoryIcon };
    }
    if (pathname === '/categories') {
      return { title: 'Categories', showBack: false, icon: CategoryIcon };
    }
    if (pathname.startsWith('/collections/')) {
      return { title: 'Collection Details', showBack: true, icon: LayersIcon };
    }
    if (pathname === '/collections') {
      return { title: 'Collections', showBack: false, icon: LayersIcon };
    }
    if (pathname === '/campaigns') {
      return { title: 'Campaigns', showBack: false, icon: ZapIcon };
    }
    if (pathname === '/team') {
      return { title: 'Team & Roles', showBack: false, icon: UsersIcon };
    }
  ...
  return (
    <header className="sticky top-0 z-30 mt-4 flex h-16 flex-shrink-0 items-center justify-between bg-transparent px-6">
      <div className="flex flex-1 items-center">
        ...
        <div className="flex flex-1 items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.05rem] text-black uppercase dark:text-white">
              {title}
            </h1>
          </div>
        </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button className="group relative flex h-9 w-9 items-center justify-center rounded-full bg-black/5 transition-colors hover:bg-black/10 dark:bg-white/5 dark:hover:bg-white/10">
          ...
          <span className="absolute top-0 right-0 block h-2.5 w-2.5 rounded-full bg-black ring-2 ring-white dark:bg-white dark:ring-black" />
        </button>
        ...
        <Link href="/profile" className="group flex cursor-pointer items-center space-x-3">
